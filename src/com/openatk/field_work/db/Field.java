...
public class Field {
	...
	private boolean isPointInPolygon(Point tap, List<Point> vertices) {
		...
		for (int j = 0; j < vertices.size() - 1; j++) {
			if (rayCastIntersect(tap, vertices.get(j), vertices.get(j + 1))) {
				intersectCount++;
				Log.d("rayCastIntersect intersectCount++", Integer.toString(intersectCount));
			}
		}
	...
	private boolean rayCastIntersect(Point tap, Point vertA, Point vertB) {
		...
		Log.d("rayCastIntersect aY", Double.toString(aY));
		Log.d("rayCastIntersect bY", Double.toString(bY));
		Log.d("rayCastIntersect aX", Double.toString(aX));
		Log.d("rayCastIntersect bX", Double.toString(bX));
		
		Log.d("rayCastIntersect (aY-bY)", Double.toString(aY-bY));
		Log.d("rayCastIntersect (aX-bX)", Double.toString(aX-bX));

		double m = (aY - bY) / (aX - bX); // Rise over run
		double bee = (-aX) * m + aY; // y = mx + b
		double x = (pY - bee) / m; // algebra is neat!
		
		Log.d("rayCastIntersect m", Double.toString(m));
		Log.d("rayCastIntersect bee", Double.toString(bee));
		Log.d("rayCastIntersect x", Double.toString(x));
