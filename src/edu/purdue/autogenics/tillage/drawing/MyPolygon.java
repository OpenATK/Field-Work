package edu.purdue.autogenics.tillage.drawing;
...
import edu.purdue.autogenics.tillage.R;
		...
		if (polygon != null) {
			...
			for (int i = 0; i < (points.size() - 1); i++) {
				this.markers.add(map.addMarker(new MarkerOptions()
						.position(points.get(i)).icon(icon).draggable(false).anchor(0.5f, 0.5f)));
			}
			if (this.markers.size() > 0)
		...
		if (markerIndex != null) {
			...
			this.markers.add(
					...
					map.addMarker(new MarkerOptions()
							...
							.icon(iconSelected).draggable(false).anchor(0.5f, 0.5f)));
